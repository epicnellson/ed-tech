...
export default function EditCourse() {
  ...
  useEffect(() => {
    if (courseData?.success && courseData?.data) {
      ...
      setFormData({
        ...
        level: course.level || '',
        privacy: course.privacy || 'institution',
  ...
  const updateCourseMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.success) {
        addToast({ title: 'Success', description: 'Course updated successfully', type: 'success' });
  ...
  if (!isTeacher) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-gray-600 mt-2">You don't have permission to edit courses.</p>
